...
**Answer: It's important to create a new variable because it'll help differentiate our various filtering and selecting datasets better for comparison later**
...
**Answer: We have a more lax dataset (54000 rows)**
...
**Answer: The dates change formatting and R now recognizes it as a date instead of a character**
...
**Answer: The rows under the month column are being grouped by the actual month**
...
**Answer: The code is going to generate two new columns called "total_votes" and "pct_moore" with the different summarizations we are asking R to calculate**
...
**Answer: R changed the order of the dataset by "pct_moore" in descending order**
...
**Answer: I think this is more interesting than the previous order because we can which counties voted for Wes Moore, who won, the least and begin to answer the why behind that with furhter reporting**
...
**Answer: "str_to_lower" = changes all characters to lowercase and "str_to_title" = capitalizes the first letter in every word**
...
**Answer: I think the mutate statement is telling R to categorize the addresses in the data by "address" and then "state", but instead of writing the state's full name, to change it to the abbreviations listed out. If R cannot find a match for the states listed, to default to "NA" in the column.**
...
**Answer: No it doesn't make sense to me because it's not clear as to what the total is and why we're looking at it**
...
1.  For the UMD courses data, I would use mutate to create a new column calculating the seat change per term of classes that are offered in more than one term

2.  For the UMD courses data, I would use the date function to change the term column to be more understandable and readable

3.  For the UMD courses data, I would use the combine function to combine rows that have the same course title
