...
**Answer: 79,366 observations (rows) & 9 variables (columns)**
...
**Answer: first line is asking to read the database, group the data by term, and then create a new column that counts how many classes are offered in each term**
...
**Answer: instead of starting with the earliest term, the line of code changed it to start with the latest term (big to small instead of small to big)**
...
**Answer: compared to the previous codeblock, the code in this block changed the output by categorizing our data by the department and term rather than just the term; it's also in alphabetical order instead of numerical**
...
**Answer: in this block, two additional columns were created that calculated the total number of classes taught and seats available per term; and the dataset was arranged by the largest amount of seats to the least**
...
**Answer: in this block, that data is arranged to reflect certain statistics within each term: the number of classes, total amount of seats, the average number of seats, and the median number of seats. the top terms share the same term months (august and january).**
...
**Answer: in this codeblock, two columns were added to calculate the maximum and minimum number of seats per department, which resulted in two columns being added to the resulting dataset**
...
1.  Are there departments that offer more classes in the winter and summer terms than most?

2.  Why do some departments have a higher average total of seats than others?

3.  Why are the maximum amount of seats higher than total number of classes across most departments?
